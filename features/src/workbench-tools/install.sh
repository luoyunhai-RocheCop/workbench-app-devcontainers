...
function install_bgen() {
    local -r VERSION="4.1.10"
    local -r SHA256="139cedd85b203481964656e9c71c811abaf943709d5dc771a7d72335ab8ff89d"
    local -r URL="https://github.com/limix/bgen/releases/download/v$VERSION/bgen-$VERSION-Linux.sh"

    mkdir -p "$WORKDIR/bgen"
    pushd "$WORKDIR/bgen"

    curl -sSL "$URL" -o "bgen-$VERSION-Linux.sh"
    if [ "$SHA256" != "$(sha256sum "bgen-$VERSION-Linux.sh" | awk '{print $1}')" ]; then
        echo "Error: SHA256 checksum mismatch for bgen."
        exit 1
    fi

    chmod +x "bgen-$VERSION-Linux.sh"
    "./bgen-$VERSION-Linux.sh" --skip-license --prefix=/usr/local --exclude-subdir

    # Install bgen-reader only if pip3 is available
    if type pip3 > /dev/null 2>&1; then
        pip3 install --no-cache-dir bgen-reader
    fi
